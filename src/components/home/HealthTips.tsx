...
const categories = [
    {
        ...
        aria: "Explore nutrition and meal plans"
    },
    {
        icon: Activity,
        title: "Fitness",
        ...
        aria: "Explore fitness and workout routines"
    },
    {
        icon: Brain,
        ...
        aria: "Explore mental health and mindfulness"
    },
    {
        icon: ShieldCheck,
        title: "Self Care",
        ...
        <section className="py-32 bg-slate-50 dark:bg-slate-900/50 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="max-w-3xl mx-auto text-center mb-20">
                    <div className="text-primary font-bold text-sm uppercase tracking-widest mb-4">Holistic Approach</div>
                    <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tight">Health Highlights</h2>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {categories.map((cat, idx) => (
                        <motion.div
                            ...
                            whileHover={{ y: -12 }}
                            className={`group bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-sm hover:shadow-2xl border border-border transition-all duration-300 ${cat.border} cursor-pointer`}
                            role="button"
                            tabIndex={0}
                            aria-label={cat.aria}
                        >
                            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 ${cat.color} group-hover:scale-110 transition-transform shadow-sm`}>
                                <cat.icon className="w-8 h-8" aria-hidden="true" />
                            </div>
                            <h3 className="text-2xl font-black mb-4 group-hover:text-primary transition-colors tracking-tight">{cat.title}</h3>
                            <p className="text-muted-foreground leading-relaxed font-medium text-[15px]">{cat.desc}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
